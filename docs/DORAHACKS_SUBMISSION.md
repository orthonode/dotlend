...
Complete zero-knowledge solvency proof pipeline built in Noir (UltraHonk). The circuit proves `total_collateral_value >= total_debt` without revealing individual positions. Proof submitted on-chain via SolvencyGateway every 30 minutes on Railway. Covers both vDOT and WPAS collateral vaults.
